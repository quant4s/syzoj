...
app.get('/api/v3/find_user', async (req, res) => {
  try {
...
// Sign up
app.get('/api/v3/sign_up', async (req, res) => {
  if (res.locals.user) {
    res.render('error', {
...
// Logout
app.post('/api/v3/logout', async (req, res) => {
...
// 排名
app.get('/api/v3/ranklist', async (req, res) => {
  try {
...
// const order = req.query.order || syzoj.config.sorting.ranklist.order;
    const sort =  syzoj.config.sorting.ranklist.field
    const order = syzoj.config.sorting.ranklist.order
